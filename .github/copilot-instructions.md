# Copilot Instructions — Favorite View Plugin
...
Jenkins plugin that provides two alternative tab bar implementations for views.
Users can mark views as favorites (shown as tabs, others in an overflow menu)
or sort view tabs via drag-and-drop on a per-user basis.

- Language: **Java 21+** (Maven build, Jenkins HPI packaging)
- UI: Jelly views under `src/main/resources/`, CSS/JS for tab interactions

## Issue linking

- Reference a Jira issue as `JENKINS-XXXXX` with a link to
...
- The PR title is used as the **changelog entry** — write it in imperative mood
  (e.g. "Add drag-and-drop reordering for view tabs").
- Follow the style of
  https://github.com/jenkins-infra/jenkins.io/blob/main/content/_data/changelogs/weekly.yml
...
| New public classes/fields/methods | Annotate with `@Restricted` |
| Deprecations | Use `@Deprecated` or `@Deprecated(forRemoval = true)` |
